...
const UploadCloudIcon = ({ className, strokeWidth }: any) => (
...
const FileCheckIcon = ({ className, strokeWidth }: any) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round">
...
const MapPinIcon = ({ className, strokeWidth }: any) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round">
...
const BotIcon = ({ className, strokeWidth }: any) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round">
...
const ShieldCheckIcon = ({ className, strokeWidth }: any) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round">
...
// Step 01 Special Visual: Forensic Timeline
  const renderStep01Visual = () => {
    // Determine the exact spot to stop the line:
    // We want it to reach the 3rd node (index 2) but stop before the 4th node (index 3).
    const stopProgress = 0.5; // Exactly 50% covers the first 3 nodes (0, 0.25, 0.5) out of 5 checks

    // Icons to use for each step
    const checks = [
      { id: 'upload', label: 'Image Uploaded', icon: UploadCloudIcon, active: true },
      { id: 'property', label: 'Confirming Registered Property', icon: FileCheckIcon, active: true },
      { id: 'gps', label: 'Validating GPS & Timestamp', icon: MapPinIcon, active: true },
      { id: 'ai', label: 'Running AI Forgery Detection', icon: BotIcon, active: false },
      { id: 'cleared', label: 'Forensics Cleared', icon: ShieldCheckIcon, active: false },
    ];

    return (
      // Natural height (no aspect ratio) for a shorter vertical card feel, wider
      <div className="relative w-full max-w-[700px] mx-auto bg-[#F8F9FC] border border-[#E8EEFF] rounded-2xl flex items-center p-8 px-12 overflow-hidden shadow-sm">
        {/* Container for the timeline: natural height via gap */}
        <div className="relative w-full ml-4 flex flex-col gap-8">
          {/* The static background line (starts/ends exactly at the 18px center of the 36px nodes) */}
          <div className="absolute left-[15px] top-[18px] bottom-[18px] w-1.5 bg-[#DDE2F0] rounded-full" />

          {/* The animated liquid glass blue line (pauses midway) */}
          <motion.div
            className="absolute left-[15px] top-[18px] bottom-[18px] w-1.5 rounded-full origin-top z-0"
            style={{
              background: 'linear-gradient(135deg, rgba(79, 142, 247, 0.95) 0%, rgba(26, 63, 212, 0.95) 100%)',
              backdropFilter: 'blur(12px)',
              boxShadow: 'inset 0 0 8px rgba(255, 255, 255, 0.4)',
              border: '1px solid rgba(255, 255, 255, 0.3)',
            }}
            initial={{ scaleY: 0 }}
            animate={inView ? { scaleY: stopProgress } : {}} // Stops halfway down
            transition={{ duration: 2, ease: 'easeOut' }}
          />

          {/* Nodes and Text */}
          {checks.map((check, i) => {
            // Delay based on line's progress to the stop point (2 seconds total to reach halfway)
            const targetLineProgress = i / (checks.length - 1);
            const willAnimate = targetLineProgress <= stopProgress;
            // Map the target line progress to the actual animation duration (2s for 0.5 progress)
            const delay = willAnimate ? (targetLineProgress / stopProgress) * 2 : 0;
            const IconComponent = check.icon;

            return (
              <div key={check.id} className="relative flex items-center gap-6 z-10 w-full">
                {/* The Node */}
                <div className="relative flex items-center justify-center w-9 h-9 shrink-0">
                  ...
                  <div className="absolute w-7 h-7 rounded-full bg-white border-[2.5px] border-[#DDE2F0] flex items-center justify-center shadow-sm">
                    <IconComponent className="w-3.5 h-3.5 text-[#8C98B2]" strokeWidth={2.5} />
                  </div>
                  
                  {/* Active liquid glass state (only renders if it's supposed to animate) */}
                  {willAnimate && (
                    <motion.div
                      ...
                      style={{
                        background: 'linear-gradient(135deg, rgba(79, 142, 247, 0.95) 0%, rgba(26, 63, 212, 0.95) 100%)',
                        backdropFilter: 'blur(12px)',
                        boxShadow: '0 8px 16px rgba(26, 63, 212, 0.25), inset 0 0 10px rgba(255, 255, 255, 0.4)',
                ...
                <div className="flex-1">
                  <motion.p
                    ...
                    initial={{ color: '#8C98B2' }} // muted
                    animate={inView && willAnimate ? { color: '#0A1628' } : {}} // dark if active
  ...
  const renderStep02Visual = () => {
    ...
    const damageZonePath = "M 48,30 C 55,26 72,25 82,33 C 88,40 85,56 80,66 C 74,74 58,77 50,70 C 43,63 42,40 48,30 Z";
        ...
        <div className="absolute inset-0 rounded-2xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.08)]">
          ...
          <motion.div
            ...
            style={{
              ...
              background: 'linear-gradient(to bottom, transparent, rgba(147, 197, 253, 0.4) 50%, transparent)',
          ...
          >
            ...
            <div
              ...
              style={{ background: 'linear-gradient(135deg, #4F8EF7 0%, #1A3FD4 100%)' }}
            >
              <motion.div
  ...
  const renderStep03Visual = () => {
    const suppliers = [
      { name: 'AutofactorNG', rating: 4.4, reviews: 59, isOpen: true, price: '₦145,000', isBest: false },
      { name: 'Ladipo Auto Market', rating: 4.1, reviews: 539, isOpen: true, price: '₦152,000', isBest: false },
      { name: 'Partste', rating: 5.0, reviews: 72, isOpen: true, price: '₦128,500', isBest: true },
      { name: 'Ugo Mech Auto Spare Parts Nig.', rating: 3.9, reviews: 28, isOpen: true, price: '₦141,500', isBest: false },
      { name: 'MyParts Nigeria', rating: 4.2, reviews: 41, isOpen: true, price: '₦138,000', isBest: false },
    ];
          ...
          {Array.from({ length: 5 }, (_, i) => (
            <svg key={i} viewBox="0 0 20 20" className="w-[14px] h-[14px]">
              <path
                ...
                fill={i < fullStars ? '#F59E0B' : (i === fullStars && hasHalf) ? 'url(#halfGold)' : '#DDE2F0'}
                stroke="none"
              />
              {i === fullStars && hasHalf && (
                <defs>
    ...
    const SupplierCard = ({ supplier, delay }: { supplier: typeof suppliers[0]; delay: number }) => (
      <motion.div
      ...
      >
        <div
          ...
          style={{
            border: supplier.isBest ? '1.5px solid rgba(79, 142, 247, 0.4)' : '1px solid rgba(0, 0, 0, 0.06)',
        ...
        >
          ...
          {supplier.isBest && (
            <div className="h-[3px] w-full" style={{ background: 'linear-gradient(90deg, #4F8EF7 0%, #1A3FD4 100%)' }} />
          )}
          ...
          <div className="px-6 py-5">
            ...
            <div className="flex items-center justify-between mb-2.5">
              <div className="flex items-center gap-2">
                <p className="text-[15px] font-bold text-[#0A1628] leading-none">{supplier.name}</p>
                ...
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" className="w-[8px] h-[8px]">
                ...
                <span
                  ...
                  style={{
                    background: supplier.isOpen ? 'rgba(16, 185, 129, 0.1)' : 'rgba(239, 68, 68, 0.08)',
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[13px] font-semibold text-[#0A1628]">{supplier.rating.toFixed(1)}</span>
              <span className="text-[11px] text-[#6B7799]">({supplier.reviews})</span>
            </div>

            {/* Row 3: Price */}
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] uppercase tracking-wider font-medium text-[#6B7799] mb-1">Front Bumper — C300</p>
                <p className={`text-xl font-bold leading-none ${supplier.isBest ? 'text-[#1A3FD4]' : 'text-[#0A1628]'}`}>
                  {supplier.price}
                </p>
              </div>
              {supplier.isBest && (
                <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: 'rgba(26, 63, 212, 0.08)' }}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="#1A3FD4" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
                    <path d="M20 6L9 17l-5-5" />
                  </svg>
                </div>
              )}
      ...
      <div className="w-full flex flex-col items-center gap-5 max-w-2xl mx-auto">
        {claims.map((claim, i) => (
          <motion.div
            key={claim.vehicle}
            initial={{ opacity: 0, y: 16 }}
            animate={inView ? { opacity: claim.isApproved ? 1 : 0.7, y: 0 } : {}}
            transition={{ delay: 0.6 + i * 0.2, duration: 0.5, ease: [0, 0, 0.2, 1] }}
            className="w-full"
          >
            <div
              className="rounded-2xl bg-white overflow-hidden"
              style={{
                border: claim.isApproved ? '1.5px solid rgba(79, 142, 247, 0.35)' : '1px solid rgba(0, 0, 0, 0.06)',
              ...
              <div className="px-7 py-6">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <p className="text-[16px] font-bold text-[#0A1628] leading-none">{claim.vehicle}</p>
                    <span className="text-[11px] text-[#6B7799] font-medium">{claim.component}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div
                      className="w-[7px] h-[7px] rounded-full"
                      style={{ background: claim.isApproved ? '#10B981' : '#F59E0B' }}
                    />
                    <span
                      className="text-[11px] font-bold uppercase tracking-wider"
                      style={{ color: claim.isApproved ? '#059669' : '#D97706' }}
                    >
                      {claim.outcome}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-8 mb-4">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider font-medium text-[#6B7799] mb-1">Confidence</p>
                    <p className="text-[20px] font-bold text-[#0A1628] leading-none">{claim.confidence}%</p>
                  </div>
                  <div className="w-[1px] h-8 bg-[#E8EEFF]" />
                  <div>
                    <p className="text-[10px] uppercase tracking-wider font-medium text-[#6B7799] mb-1">Estimate</p>
                    <p className="text-[20px] font-bold text-[#0A1628] leading-none">{claim.estimate}</p>
                  </div>
                  <div className="w-[1px] h-8 bg-[#E8EEFF]" />
                  <div>
                    <p className="text-[10px] uppercase tracking-wider font-medium text-[#6B7799] mb-1">Severity</p>
                    <p className={`text-[20px] font-bold leading-none ${claim.severity === 'Moderate' ? 'text-[#E2A816]' : 'text-[#E25C5C]'}`}>{claim.severity}</p>
                  </div>
                </div>
                ...
                >
                  {claim.isApproved ? (
                    <svg viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 shrink-0">
                      <path d="M20 6L9 17l-5-5" />
                    </svg>
                  ) : (
                    <svg viewBox="0 0 24 24" fill="none" stroke="#D97706" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 shrink-0">
                      <path d="M5 12h14M12 5l7 7-7 7" />
                    </svg>
                  )}
                  <p className="text-[13px] text-[#2D3A5C] font-semibold">{claim.outcomeDesc}</p>
                </div>
              </div>
            </div>
          </motion.div>
    ...
    return (
      <div className="w-full flex flex-col items-center gap-4 max-w-md mx-auto">
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #4F8EF7, #1A3FD4)' }}>
                <span className="text-[12px] text-white font-bold">D</span>
              </div>
              <div>
                <span className="text-[10px] font-bold text-[#0A1628]/40 tracking-wider uppercase">DERA PARTNER</span>
                <p className="text-[10px] text-[#6B7799]">now</p>
              </div>
            </div>
            <p className="text-[15px] font-bold text-[#0A1628] leading-snug mb-1">New Job Request</p>
            <p className="text-[13px] text-[#3D4A6B] leading-snug">Benz C300 • Front Bumper • Lekki Phase 1</p>
          </div>
        </motion.div>
          ...
          >
            <div className="px-6 py-6">
              <div className="flex items-center justify-between mb-5">
                <p className="text-[18px] font-bold text-[#0A1628]">Benz C300</p>
                <span className="text-[10px] font-bold text-[#059669] bg-[rgba(16,185,129,0.1)] px-3 py-1 rounded-full tracking-wider">APPROVED</span>
              </div>
              <div className="flex items-center gap-8 mb-5">
                <div>
                  <p className="text-[10px] uppercase tracking-wider font-medium text-[#6B7799] mb-1">Component</p>
                  <p className="text-[16px] font-bold text-[#0A1628]">Front Bumper</p>
                </div>
                <div className="w-[1px] h-8 bg-[#E8EEFF]" />
                <div>
                  <p className="text-[10px] uppercase tracking-wider font-medium text-[#6B7799] mb-1">Location</p>
                  <p className="text-[16px] font-bold text-[#0A1628]">Lekki Phase 1</p>
                </div>
              </div>
              <div className="flex items-center gap-8 pt-5" style={{ borderTop: '1px solid rgba(0,0,0,0.05)' }}>
                <div>
                  <p className="text-[10px] uppercase tracking-wider font-medium text-[#6B7799] mb-1">Payout</p>
                  <p className="text-[22px] font-bold text-[#0A1628]">₦184,500</p>
                </div>
                <div className="w-[1px] h-8 bg-[#E8EEFF]" />
                <div>
                  <p className="text-[10px] uppercase tracking-wider font-medium text-[#6B7799] mb-1">ETA</p>
                  <p className="text-[22px] font-bold text-[#0A1628]">25 min</p>
                </div>
              </div>
            </div>
            <div className="px-6 pb-6 flex gap-3">
              <div className="flex-1 py-3.5 rounded-xl text-center" style={{ background: 'linear-gradient(to bottom, #2A2A2A, #000)' }}>
                <span className="text-[13px] text-white font-bold">Accept Job</span>
              </div>
              <div className="flex-1 py-3.5 rounded-xl bg-[#F0F1F5] text-center">
                <span className="text-[13px] text-[#0A1628] font-bold">Decline</span>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          style={{ background: 'rgba(0, 0, 0, 0.03)', border: '1px solid rgba(0, 0, 0, 0.03)' }}
        />
        <motion.div
          initial={{ opacity: 0 }}
          animate={inView ? { opacity: 1 } : {}}
            ...
            >
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <p className="text-[17px] font-semibold text-[#0A1628] leading-none mt-0.5">Benz C300 Analysis</p>
                </div>
                ...
                <div className="text-[15px] text-[#3D4A6B] leading-relaxed space-y-1.5 ml-11">
                  ...
                  <p><span className="font-medium text-[#0A1628]">Severity:</span> Moderate</p>
                  <p><span className="font-medium text-[#0A1628]">Confidence:</span> 94.2%</p>
                </div>
              </div>
            </motion.div>
          </div>
